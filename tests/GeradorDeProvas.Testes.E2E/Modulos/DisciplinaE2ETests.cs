...
public sealed class DisciplinaE2ETests : E2ETestsBase
{
    ...
    public async Task DeveExibir_ListagemVazia_ParaUsuario_SemDisciplinas()
    {
        ...
        // Act
        await Page.GotoAsync($"{UrlBase}/Disciplina/Listar");
        ...
        await Expect(Page.GetByRole(AriaRole.Heading, new() { Name = "Listagem de Disciplinas" }))
            .ToBeVisibleAsync();

        await Expect(Page.GetByRole(AriaRole.Link, new() { Name = "Cadastrar Nova" }))
            .ToBeVisibleAsync();

        await Expect(Page.GetByText("Nenhuma disciplina cadastrada.", new() { Exact = true }))
            .ToBeVisibleAsync();
    }
    ...
    public async Task DeveCadastrar_Disciplina_ComDadosValidos()
    {
        ...
        await Expect(Page.GetByText("Matemática", new() { Exact = true }))
            .ToBeVisibleAsync();
    ...
    public async Task DeveEditar_Disciplina_ComDadosValidos()
    {
        ...
        await Expect(Page.GetByText("História do Brasil", new() { Exact = true }))
            .ToBeVisibleAsync();
    ...
    public async Task DeveExcluir_Disciplina_SemVinculos()
    {
        ...
        // Act
        await Expect(Page.GetByText("Deseja realmente excluir esta disciplina?", new() { Exact = true }))
            .ToBeVisibleAsync();
    ...
    {
        await Page.GotoAsync($"{UrlBase}/Disciplina/Cadastrar");
